 ...
 * @version 1.0.0
 * @class DimenUtils
 * @describe 根据values/dimens.xml, 自动计算比例并生成不同分辨率的dimens.xml,
 * 注意用dp和sp，不要用dip，否则生成可能会出错；xml值不要有空格.
 * 1. 320，针对以下屏幕配置的设备：240x320ldpi（QVGA手持设备）、 320x480mdpi（手持设备）、480x800hdpi（高分辨率手持设备）；
 * 2. 480，针对480x800mdpi的屏幕（平板或手持设备）；
 * 3. 600，针对600x1024mdip的屏幕（7英寸平板）；
 * 4. 720，针对720x1280mdip的屏幕（10英寸平板）
 * 可参考：http://blog.csdn.net/persuit/article/details/7663574
...
public class DimenUtils {
    ...
    public static void gen() {
        ...
        try {
            ...
            String sw480file = "./app/src/main/res/values-sw480dp-land/dimens.xml";//land横屏显示，port竖屏显示
